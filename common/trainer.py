...
class Trainer(AbstractTrainer):
   ...
   and some other features helpful for model training and evaluation.
    ...
    def _generate_train_loss_output(self, epoch_idx, s_time, e_time, losses):
        train_loss_output = 'epoch %d training [time: %.4fs, ' % (epoch_idx, e_time - s_time)
        if isinstance(losses, tuple):
    ...
    def fit(self, train_data, valid_data=None, test_data=None, saved=False, verbose=True):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if epoch_idx == 0:
                post_info = self.model.post_epoch_processing(epoch_idx, valid_data.inter_matrix())
            else:
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                valid_end_time = time()
                valid_score_output = "epoch %d evaluating [time: %.4fs, valid_score: %f]" % \
        ...
        if verbose:
            ...
            self.logger.info(f'Total training time (excluding preprocessing): {total_training_time:.4f} seconds ({total_training_time/60:.4f} minutes)')
            self.logger.info(f'Training epochs: {epoch_idx + 1} epochs')
            self.logger.info('=' * 50)
